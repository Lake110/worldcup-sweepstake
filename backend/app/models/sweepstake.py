...
class Sweepstake(Base):
    ...
    participants = relationship(
        "Participant", back_populates="sweepstake", cascade="all, delete"
    )
...
class Participant(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    sweepstake_id = Column(
        UUID(as_uuid=True), ForeignKey("sweepstakes.id"), nullable=False
    )
    user_id = Column(
        UUID(as_uuid=True), ForeignKey("users.id"), nullable=True
    )  # nullable for quick draw
    ...
    assignments = relationship(
        "TeamAssignment", back_populates="participant", cascade="all, delete"
    )
